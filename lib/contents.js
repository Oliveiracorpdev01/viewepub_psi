...
class Contents {
  ...
  detectBrowser() {
    ...
    if (userAgent.indexOf('chrome') > -1) {
      if (userAgent.indexOf('edg') > -1) {
        ...
        // 'Opera';
        return localStorage.getItem('psiqueasy-epub-zoom');
      } else {
        // 'Google Chrome';
        return "100%";
      }
  ...
  fit(width, height, section) {
    ...
    this.css("background-color", "white"); // se for chrome ou edge, seta o zoom para 100% se não pega o zoom do localStorage

    this.css("zoom", this.detectBrowser()); //"100%" ou localStorage.getItem('psiqueasy-epub-zoom')
